...
```
  - [ ] 코치 이름이 2글자 미만 4글자 초과일 경우 : ```코치의 이름은 최소 2글자, 최대 4글자로 입력해야 합니다.```
  - [ ] 코치 명수가 2명 미만, 5명 초과일 경우 : ```코치는 최소 2명, 최대 5명 입력해야 합니다.```
<br>

- [ ] 각 코치가 못 먹는 메뉴를 입력받는다. (, 로 구분해서 입력한다.)
  - [ ] 모든 코치 반복
...
Exception이 아닌 IllegalArgumentException, IllegalStateException 등과 같은 명확한 유형을 처리한다.
```
  - [ ] 해당 코치가 못먹는 음식 수가 음수거나 3개 이상일 경우 : ```각 코치는 최소 0개, 최대 2개의 못 먹는 메뉴를 입력해야 합니다.```
  - [ ] 존재하지 않는 음식이 포함된 경우 : ```존재하지 않는 음식이 입력되었습니다.```
 
## 메뉴 추천
- [ ] 월요일부터 금요일까지 점심 메뉴를 정한다.
  - [ ] 추천할 카테고리를 무작위로 정한다.
    ...
    - 한 주에 같은 카테고리는 최대 2회까지만 고를 수 있다.
      - [ ] 각 카테고리가 몇번 선택되었는지 저장하고, 확인 후 2번이상일 경우 카테고리를 다시 추천한다.
  - [ ] 각 코치가 해당요일에 먹을 메뉴를 추천한다.
    - [ ] 추천할 메뉴는 정해진 카테고리에 있는 메뉴를 camp.nextstep.edu.missionutils에서 제공하는 Randoms.shuffle()을 통해 임의의 순서로 섞은 후, 첫 번째 값을 사용해야 한다.
    ```
       String menu = Randoms.shuffle(menus).get(0);
    ```
    - [ ] 각 코치별로 못먹는 음식이 나올 경우 다시 추천한다.
    - 각 코치에게 한 주에 중복되지 않는 메뉴를 추천해야 한다.
      - [ ] 코치별로 추천된 음식을 저장하고, 해당 음식이 또 추천될 경우 다시 추천한다. (추천할 수 없는 메뉴인 경우 다시 섞은 후 첫 번째 값을 사용해야 한다.)
