...
## 입력 
- [x] 비상 근무 배정 월과 시작 요일 입력 (쉼표로 구분 : 5,월)
  - 월(숫자)과 시작 요일(일, 월, 화, 수, 목, 금, 토) 정보를 입력받는다.
  - 연도는 고려하지 않으며, 매년 2월은 28일까지만 있다고 가정한다.<br>
...
"[ERROR] 유효하지 않은 입력 값입니다. 다시 입력해 주세요." 출력 후 "월, 요일"부터 입력을 다시 받는다.
```
  - [x] 월,요일 외에 다른 문자 입력했을 경우
  - [x] 월이 1월 미만, 12월 초과일 경우
  - [x] 요일이 (월,화,수,목,금)이 아닐 경우
<br>
...
```
사용자가 잘못된 값을 입력할 경우 IllegalArgumentException를 발생시키고,
"[ERROR] 유효하지 않은 입력 값입니다. 다시 입력해 주세요." 에러 메시지를 출력 후 "평일 순번"부터 입력을 다시 받는다.
```
  - [x] 닉네임이 5자 초과일 경우
  - [x] 닉네임이 중복될 경우
  - [x] 비상 근무 사원이 5명 미만, 35명 초과 일 경우
  - [x] 평일 비상 근무 사원에 없는 닉네임이 존재할 경우
<br>

## 근무 배정
- [x] 순번에 따라 비상 근무일을 배정한다.
  - [x] 휴일(토요일, 일요일, 법정공휴일) 체크 후 휴일일 경우, 휴일 순번에 따라 배정
    - x ] 평일이면서 법정 공휴일의 경우 요일 뒤에 (휴일) 표기
  - [x] 휴일이 아닐 경우, 평일 순번에 따라 배정
- [x] 비상 근무자는 어떤 경우에도 연속 2일은 근무할 수 없다.
  - [x] 해당 배정 사원이 전날 배정되었는지 확인, 배정되었을 경우 뒷 순번이라 교체
  - [x] 교체한 경우 기록 필수
...
- [x] 비상 근무자는 평일 순번, 휴일 순번에 각각 1회 편성되어야 한다.
  - [x] 한 명이 평일 두번 혹은 휴일 두번 편성되면 안된다.
- [x] 비상 근무자는 어떤 경우에도 연속 2일은 근무할 수 없다.
